                            ...
                            <ul class="breadcrumb-elements">
                                <li class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        @if (count($data)==0)
                                            <li><a href="{{ route('jumlah-murid.create',array($kategori->id,$tahun)) }}"><i class="icon-googleplus5 pull-right"></i> Tambah Data</a></li>
                                        @else
                                            <li><a href="{{ route('jumlah-murid.edit',array($kategori->id,$tahun)) }}"><i class="icon-googleplus5 pull-right"></i> Ubah Data</a></li>
                                        @endif
                                    </ul>
                                </li>
                            </ul>
                                ...
                                <tr>
                                    ...
                                    @foreach ($jenjang as $idxjg=> $jg)
                                        @php
                                            ...
                                            {
                                                $pr=$data[$item->id][$idxjg]['jumlah_perempuan'];
                                            }
                                            if(isset($data[$item->id][$idxjg]['jumlah_laki']))
                                            {
                                                $lk=$data[$item->id][$idxjg]['jumlah_laki'];
                                            }
                                            $jlh_lk[$idxjg][]=$lk;
                                            $jlh_pr[$idxjg][]=$pr;
                                        @endphp
                                        <td class="text-center">{{number_format($lk,0,',','.')}}</td>
                                        <td class="text-center">{{number_format($pr,0,',','.')}}</td>
                                    @endforeach
                                </tr>
                            @endforeach
                            <tr>
                                <th colspan="2" class="text-right">JUMLAH SISWA</th>
                                @foreach ($jenjang as $idxjg=> $jg)
                                    <th class="text-center">{{isset($jlh_lk[$idxjg]) ? number_format(array_sum($jlh_lk[$idxjg]),0,',','.') : 0}}</th>
                                    <th class="text-center">{{isset($jlh_pr[$idxjg]) ? number_format(array_sum($jlh_pr[$idxjg]),0,',','.') : 0}}</th>
                                @endforeach
                            </tr>
                        </tbody>
                    </table>
...
@section('footscript')
    ...
    function loaddata(tahun)
        {
            location.href='{{url("/pendidikan/murid-jenjang-pendidikan/".$id_kategori)}}/'+tahun;
        }
	</script>
@endsection