    ...
    <div class="content">
        <div class="panel panel-flat">
            ...
            <div class="panel-body">
                <form action="{{ route('jumlah-murid.store', array($kategori->id,$tahun)) }}" method="post">
                    @csrf
                            ...
                            @foreach ($kecamatan as $key => $item)
                                <tr>
                                    ...
                                    @foreach ($jenjang as $idxjg=> $jg)
                                        <td class="text-center"><input type="text" class="form-control text-right" value="0" name="jumlah_laki[{{$item->id}}][{{$idxjg}}]"></td>
                                        <td class="text-center"><input type="text" class="form-control text-right" value="0" name="jumlah_perempuan[{{$item->id}}][{{$idxjg}}]"></td>
                                    @endforeach
                                </tr>
                            @endforeach
                        </tbody>
