                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    @if ($tahun_tersedia->count()!=0)
                                        @foreach ($tahun_tersedia as $item)
                                            <li><a href="{{ route('kependudukan-sampling.index', [$kategori->id, $item]) }}">{{ $item }}</a></li>
                                        @endforeach
                                    @else
                                        <li><a href="{{ route('kependudukan-sampling.index', [$kategori->id, date('Y')]) }}">{{ date('Y') }}</a></li>
                                    @endif
                                </ul>
                            </li>
                            <li class="dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    <li><a href="{{ route('kependudukan-sampling.create', [$kategori->id, $tahun]) }}">Tambah Data Baru</a></li>
                                    <li><a href="{{ route('kependudukan-sampling.edit', [$kategori->id, $tahun]) }}">Ubah Data {{ $tahun }}</a></li>
                                </ul>
                            </li>
                        </ul>
                    </div>
    ...
    <script>
		...
		var horizontalBarChartData = {
			...
			datasets: [{
				...
				borderWidth: 1,
				data: [
					@foreach($chart_umur_4 as $item)
                        {{ $item }},
                    @endforeach
				]
			}, {
				label: 'Umur 0-17 Tahun',
				backgroundColor: color(window.chartColors.blue).alpha(0.5).rgbString(),
				borderColor: window.chartColors.blue,
				borderWidth: 1,
				data: [
					@foreach($chart_umur_17 as $item)
                        {{ $item }},
                    @endforeach
				]
			}]
