    ...
    <div class="content">
        <div class="panel panel-flat">
            ...
            <div class="panel-body">
                <form action="{{ route('politik-status.update', [$kategori->id, $tahun]) }}" method="post">
                    @csrf
                    @method('put')
                        ...
                        <tbody>
                            @foreach ($data as $key => $item)
                                <tr>
                                    ...
                                    <td>
                                        <input type="number" min="0" class="form-control" name="tidak_bekerja[]" value="{{ $item->tidak_bekerja }}" required>
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control" name="bekerja[]" value="{{ $item->bekerja }}" required>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
