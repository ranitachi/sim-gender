            ...
            <div class="content-group">
                <div class="page-header page-header-default" style="border: 1px solid #ddd; border-bottom: 0;">
                    ...
                    <div class="breadcrumb-line"><a class="breadcrumb-elements-toggle"><i class="icon-menu-open"></i></a>
                        ...
                        <a href="{{route('status-pendidikan.index',[$id_kategori,$tahun])}}" class="btn btn-xs btn-primary pull-right" style="margin-top:5px;"><i class="icon-arrow-left52"></i> Kembali</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="content">
        <div class="panel panel-flat">
            ...
            <div class="panel-body">
                <form action="{{ route('status-pendidikan.update', array($kategori->id,$tahun)) }}" method="post">
                    @csrf
                        ...
                        <thead>
                            <tr>
                                ...
                                @foreach ($status as $idx=>$item)
                                    @php
                                        list($f1,$f2)=explode('_',$idx);
                                        if($f1=='sekolah')
                                            $subtxt='Masih Bersekolah<br>';
                                        else
                                            $subtxt='';
                                    @endphp
                                    <th class="text-center">{!!$subtxt!!}{{$item}}</th>
                                @endforeach
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <th colspan="{{(count($status) + 1)}}"><b>Jenis Kelamin</b></th>
                            </tr>
                            <tr>
                                <td class="text-right">Laki Laki</td>
                                @foreach ($status as $idx_jen=>$item)
                            ...
                            <tr>
                                <th colspan="{{(count($status) + 1)}}"><b>Kuantal Pengeluaran</b></th>
                            </tr>
                            @for ($i = 1; $i <= 5; $i++)
                                <tr>
                            ...
                            <tr>
                                ...
                                @foreach ($status as $idx_jen=>$item)
                                    <td class="text-center">
                                        <input type="text" class="form-control text-center" name="kab_tangerang[{{$idx_jen}}]" value="{{isset($data[$idx_jen]['kab_tangerang']) ? number_format($data[$idx_jen]['kab_tangerang'],2,',','.') : 0}}"> 
                                    </td>
                                @endforeach
                            </tr>
                        </tbody>
