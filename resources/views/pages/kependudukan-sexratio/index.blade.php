...
@section('title')
    <title>Kependudukan - Sex Ratio : Sistem Informasi Statistik Kabupaten Tangerang Tahun {{$tahun}}</title>
                            ...
                            <ul class="breadcrumb-elements">
                                <li class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        @if (count($data)==0)
                                            <li><a href="{{ route('sex-ratio.create', [$kategori->id,$tahun]) }}"><i class="icon-googleplus5 pull-right"></i> Tambah Data</a></li>
                                        @else
                                            <li><a href="{{ route('sex-ratio.edit', [$kategori->id,$tahun]) }}"><i class="icon-googleplus5 pull-right"></i> Ubah Data</a></li>
                                        @endif
                                    </ul>
                                </li>
                            </ul>
                    ...
                    <tbody>
                        ...
                        @foreach ($kecamatan as $item)
                            <tr>
                                ...
                                <td class="text-right">{{isset($data[$item->id]['laki_laki']) ? number_format($data[$item->id]['laki_laki'],0,',','.') : '-'}}</td>
                                <td class="text-right">{{isset($data[$item->id]['perempuan']) ? number_format($data[$item->id]['perempuan'],0,',','.') : '-'}}</td>
                                <td class="text-right">{{isset($data[$item->id]['jumlah']) ? number_format($data[$item->id]['jumlah'],0,',','.') : '-'}}</td>
                                <td class="text-center">{{isset($data[$item->id]['sex_ratio']) ? $data[$item->id]['sex_ratio'] : '-'}}</td>
...
@section('footscript')
    <script>
        ...
        {
            location.href=APP_URL+'/kependudukan/sex-ratio/{{$kategori->id}}/'+tahun;
        }
    </script>
