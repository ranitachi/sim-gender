                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    @if ($tahun_tersedia->count()!=0)
                                        @foreach ($tahun_tersedia as $item)
                                            <li><a href="{{ route('kependudukan-perempuan.index', [$kategori->id, $item]) }}">{{ $item }}</a></li>
                                        @endforeach
                                    @else
                                        <li><a href="{{ route('kependudukan-perempuan.index', [$kategori->id, date('Y')]) }}">{{ date('Y') }}</a></li>
                                    @endif
                                </ul>
                            </li>
                            <li class="dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    <li><a href="{{ route('kependudukan-perempuan.create', [$kategori->id, $tahun]) }}">Tambah Data Baru</a></li>
                                    <li><a href="{{ route('kependudukan-perempuan.edit', [$kategori->id, $tahun]) }}">Ubah Data {{ $tahun }}</a></li>
                                </ul>
                            </li>
                        </ul>
                    </div>
    ...
    <div class="content">
        ...
        <div class="panel panel-flat">
            ...
            @foreach ($data as $key => $item)
                @php
                    $ekonomis += $item->ekonomi;
                    $psikologis += $item->psikologis;
                @endphp
            @endforeach

            <div style="margin:20px 0 0 20px;">
                <h6>Total Jumlah Perempuan Rawan Sosial Ekonomi : &nbsp;&nbsp;<strong>{{ $ekonomis }}</strong></h6>
                <h6>Total Jumlah Keluarga Bermasalah Sosial Psikologis : &nbsp;&nbsp;<strong>{{ $psikologis }}</strong></h6>
            </div>
            ...
            <table class="table datatable-basic">
                <thead>
                    <tr>
                        ...
                        <th>Kecamatan</th>
                        <th>Perempuan Rawan Sosial Ekonomis</th>
    ...
    <script>
		...
		var horizontalBarChartData = {
			...
			datasets: [{
				...
				borderWidth: 1,
				data: [
					@foreach($chart_psikologis as $item)
                        {{ $item }},
                    @endforeach
				]
			}]
