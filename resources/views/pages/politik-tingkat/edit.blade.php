    ...
    <div class="content">
        <div class="panel panel-flat">
            ...
            <div class="panel-body">
                <form action="{{ route('politik-tingkat.update', [$kategori->id, $tahun]) }}" method="post">
                    @csrf
                    @method('put')
                        ...
                        <tbody>
                            @foreach ($data as $key => $item)
                                <tr>
                                    ...
                                    <td>
                                        <input type="number" min="0" class="form-control" name="tidak_sekolah[]" value="{{ $item->tidak_sekolah }}" required>
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control" name="sd[]" value="{{ $item->sd }}" required>
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control" name="sltp[]" value="{{ $item->sltp }}" required>
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control" name="slta[]" value="{{ $item->slta }}" required>
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control" name="pt[]" value="{{ $item->pt }}" required>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
