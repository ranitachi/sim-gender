                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        @if ($tahun_tersedia->count()!=0)
                                            @foreach ($tahun_tersedia as $item)
                                                <li><a href="{{ route('kependudukan-air-mandi.index', [$kategori->id, $item]) }}">{{ $item }}</a></li>
                                            @endforeach
                                        @else
                                            <li><a href="{{ route('kependudukan-air-mandi.index', [$kategori->id, date('Y')]) }}">{{ date('Y') }}</a></li>
                                        @endif
                                    </ul>
                                </li>
                                @if (Auth::check())
                                    <li class="dropdown">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-right">
                                            <li><a href="{{ route('kependudukan-air-mandi.create', [$kategori->id, $tahun]) }}">Tambah Data Baru</a></li>
                                            <li><a href="{{ route('kependudukan-air-mandi.edit', [$kategori->id, $tahun]) }}">Ubah Data {{ $tahun }}</a></li>
                                        </ul>
                                    </li>
                                @endif
                            </ul>
            ...
            <table class="table datatable-basic">
                <thead>
                    <tr>
                        ...
                        <th>Leding</th>
                        <th>Sumur/Pompa</th>
                        <th>Matar Air Terlindungi</th>
                        <th>Matar Air Tidak Terlindungi</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($data as $key => $item)
                        <tr>
                            ...
                            <td>{{ $item->isi_ulang }}</td>
                            <td>{{ $item->leding }}</td>
                            <td>{{ $item->sumur_pompa }}</td>
                            <td>{{ $item->mata_air_terlindungi }}</td>
                            <td>{{ $item->mata_air_tidak_terlindungi }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
    ...
    <script>
		...
		var horizontalBarChartData = {
			...
			datasets: [{
				...
				borderWidth: 1,
				data: [
					@foreach($chart_isi_ulang as $item)
                        {{ $item }},
                    @endforeach
				]
			}, {
				...
				borderWidth: 1,
				data: [
					@foreach($chart_sumur_pompa as $item)
                        {{ $item }},
                    @endforeach
				]
			}, {
				...
				borderWidth: 1,
				data: [
					@foreach($chart_mata_air_terlindungi as $item)
                        {{ $item }},
                    @endforeach
				]
			}, {
