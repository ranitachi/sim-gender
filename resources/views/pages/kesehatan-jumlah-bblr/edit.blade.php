    ...
    <div class="content">
        <div class="panel panel-flat">
            ...
            <div class="panel-body">
                <form action="{{ route('kesehatan-jumlah-bblr.update', array($kategori->id)) }}" method="post">
                    @csrf
                    @method('put')
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @foreach ($kecamatan as $key => $item)
                             @php
                                $gizi_buruk=$bayi_lahir=$bblr_jumlah=$bblr_dirujuk=rand(10,100);
                            ...
                            @endphp
                                <tr>
                                    ...
                                    <td>
                                        <input type="number" min="0" class="form-control text-center" name="bayi_lahir[{{$item->id}}]" value="{{$bayi_lahir}}">
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control text-center" name="bblr_jumlah[{{$item->id}}]" value="{{$bblr_jumlah}}">
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control text-center" name="bblr_dirujuk[{{$item->id}}]" value="{{$bblr_dirujuk}}">
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control text-center" name="gizi_buruk[{{$item->id}}]" value="{{$gizi_buruk}}">
                                    </td>
