                            ...
                            <ul class="breadcrumb-elements">
                                <li class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        @if (count($data)==0)
                                            <li><a href="{{ route('rasio-murid-guru.create',array($kategori->id,$tahun)) }}"><i class="icon-googleplus5 pull-right"></i> Tambah Data</a></li>
                                        @else
                                            <li><a href="{{ route('rasio-murid-guru.edit',array($kategori->id,$tahun)) }}"><i class="icon-googleplus5 pull-right"></i> Ubah Data</a></li>
                                        @endif
                                    </ul>
                                </li>
                            </ul>
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @foreach ($jenjang as $key => $item)
                                @php
                                    if(isset($data[$key]['jumlah_murid']))
                                    {
                                        $jlh_mr[]=$murid=$data[$key]['jumlah_murid'];
                                    }
                                    else
                                        $jlh_mr[]=$murid=0;

                                    if(isset($data[$key]['jumlah_guru']))
                                    {
                                        $jlh_gr[]=$guru=$data[$key]['jumlah_guru'];
                                    }
                                    else
                                        $jlh_gr[]=$guru=0;

                                    if(isset($data[$key]['rasio']))
                                    {
                                        $jlh_rasio[]=$rasio=$data[$key]['rasio'];
                                    }
                                    else
                                        $jlh_rasio[]=$rasio=0;
                                @endphp
                                <tr>
                                    ...
                                    <td>{{$item}}</td>
                                    <td class="text-center">{{number_format($murid,0,',','.')}}</td>
                                    <td class="text-center">{{number_format($guru,0,',','.')}}</td>
                                    <td class="text-center">{{number_format($rasio,2,',','.')}}</td>
                                </tr>
                            @endforeach
                            <tr>
                                <th colspan="2" class="text-right">TOTAL</th>
                                <th class="text-center">{{count($jlh_mr)!=0 ? number_format(array_sum($jlh_mr),0,',','.') : 0}}</th>
                                <th class="text-center">{{count($jlh_gr)!=0 ? number_format(array_sum($jlh_gr),0,',','.') : 0}}</th>
                                <th class="text-center">{{count($jlh_rasio)!=0 ? number_format(array_sum($jlh_rasio),2,',','.') : 0}}</th>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
@section('footscript')
    ...
    function loaddata(tahun)
        {
            location.href='{{url("/pendidikan/rasio-jenjang-pendidikan/".$id_kategori)}}/'+tahun;
        }
	</script>
@endsection