    ...
    <div class="content">
        <div class="panel panel-flat">
            ...
            <div class="panel-body">
                <form action="{{ route('politik-umur.update', [$kategori->id, $tahun]) }}" method="post">
                    @csrf
                    @method('put')
                        ...
                        <tbody>
                            @foreach ($data as $key => $item)
                                <tr>
                                    ...
                                    <td>
                                        <input type="number" min="0" class="form-control" name="umur_17[]" value="{{ $item->umur_17 }}" required>
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control" name="umur_24[]" value="{{ $item->umur_24 }}" required>
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control" name="umur_59[]" value="{{ $item->umur_59 }}" required>
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control" name="umur_60[]" value="{{ $item->umur_60 }}" required>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
