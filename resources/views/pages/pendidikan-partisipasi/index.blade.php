                            ...
                            <ul class="breadcrumb-elements">
                                <li class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        @if (count($data)==0)
                                            <li><a href="{{ route('partisipasi.create',array($kategori->id,$tahun)) }}"><i class="icon-googleplus5 pull-right"></i> Tambah Data</a></li>
                                        @else
                                            <li><a href="{{ route('partisipasi.edit',array($kategori->id,$tahun)) }}"><i class="icon-googleplus5 pull-right"></i> Ubah Data</a></li>
                                        @endif
                                    </ul>
                                </li>
                            </ul>
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @foreach ($jenjang as $key => $item)
                                @php
                                    if(isset($data[$key]['laki_laki']))
                                    {
                                        $jlh_lk[]=$lk=$data[$key]['laki_laki'];
                                    }
                                    else
                                        $jlh_lk[]=$lk=0;

                                    if(isset($data[$key]['perempuan']))
                                    {
                                        $jlh_pr[]=$pr=$data[$key]['perempuan'];
                                    }
                                    else
                                        $jlh_pr[]=$pr=0;
                                ...
                                <tr>
                                    ...
                                    <td>{{$item}}</td>
                                    <td class="text-center">{{number_format($lk,2,',','.')}}</td>
                                    <td class="text-center">{{number_format($pr,2,',','.')}}</td>
                                    <td class="text-center">{{number_format((($pr+$lk)/2),2,',','.')}}</td>
                                </tr>
                            @endforeach
                            <tr>
                                <th colspan="2" class="text-right">TOTAL</th>
                                <th class="text-center">{{count($jlh_lk)!=0 ? number_format(array_sum($jlh_lk),2,',','.') : 0}}</th>
                                <th class="text-center">{{count($jlh_pr)!=0 ? number_format(array_sum($jlh_pr),2,',','.') : 0}}</th>
                                <th></th>
                            </tr>
                        </tbody>
                    </table>
...
@section('footscript')
    ...
    function loaddata(tahun)
        {
            location.href='{{url("/pendidikan/partisipasi-jenjang-pendidikan/".$id_kategori)}}/'+tahun;
        }
	</script>
@endsection