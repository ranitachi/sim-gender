...
@section('title')
    <title>Kependudukan - Kelompok Umur : Sistem Informasi Statistik Kabupaten Tangerang Tahun {{$tahun}}</title>
                            ...
                            <ul class="breadcrumb-elements">
                                <li class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        @if (count($data)==0)
                                            <li><a href="{{ route('kelompok-umur.create', [$kategori->id,$tahun]) }}"><i class="icon-googleplus5 pull-right"></i> Tambah Data</a></li>
                                        @else
                                            <li><a href="{{ route('kelompok-umur.edit', [$kategori->id,$tahun]) }}"><i class="icon-googleplus5 pull-right"></i> Ubah Data</a></li>
                                        @endif
                                    </ul>
                                </li>
                            </ul>
                    ...
                    <tbody>
                        ...
                        @foreach ($kelompok as $item)
                            <tr>
                                ...
                                <td>{{ $item}}</td>
                                <td class="text-right">{{isset($data[$item]['laki_laki']) ? number_format($data[$item]['laki_laki'],0,',','.') : '-'}}</td>
                                <td class="text-center">{{isset($data[$item]['persen_laki_laki']) ? number_format($data[$item]['persen_laki_laki'],2,',','.') : '-'}}</td>
                                <td class="text-right">{{isset($data[$item]['perempuan']) ? number_format($data[$item]['perempuan'],0,',','.') : '-'}}</td>
                                <td class="text-center">{{isset($data[$item]['persen_perempuan']) ? number_format($data[$item]['persen_perempuan'],2,',','.') : '-'}}</td>
...
@section('footscript')
    <script>
        ...
        {
            location.href=APP_URL+'/kependudukan/kelompok-umur/{{$kategori->id}}/'+tahun;
        }
    </script>
