    ...
    <div class="content">
        <div class="panel panel-flat">
            ...
            <div class="panel-body">
                <form action="{{ route('politik-pendidikan.update', [$kategori->id, $tahun]) }}" method="post">
                    @csrf
                    @method('put')
                        ...
                        <tbody>
                            @foreach ($data as $key => $item)
                                <tr>
                                    <td>{{ $key = $key + 1 }}</td>
                                    <td>
                                        <input type="text" class="form-control" readonly name="partai[]" value="{{ $item->partai }}">
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control" name="smu[]" value="{{ $item->smu }}" required>
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control" name="d1_d2[]" value="{{ $item->d1_d2 }}" required>
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control" name="s1[]" value="{{ $item->s1 }}" required>
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control" name="s2_s3[]" value="{{ $item->s2_s3 }}" required>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
