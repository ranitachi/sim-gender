                        ...
                        <ul class="breadcrumb-elements">
                            <li class="dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    @if ($data->count()==0)
                                        <li><a href="{{ route('kesehatan-alh.create',array($kategori->id)) }}"><i class="icon-googleplus5 pull-right"></i> Tambah Data</a></li>
                                    @else
                                        <li><a href="{{ route('kesehatan-alh.edit',array($kategori->id)) }}"><i class="icon-googleplus5 pull-right"></i> Ubah Data</a></li>
                                    @endif
                                </ul>
                            </li>
                        </ul>
                ...
                <table class="table datatable-basic table-bordered table-striped">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">Anak Lahir Hidup</th>
                    ...
                    <tbody>
                        @foreach ($kecamatan as $key => $item)
                            @php
                                ...
                                $jumlah=$jlh_alh+$jlh_penolong;
                            @endphp
                            <tr>
                                <td class="text-center">{{ $key = $key + 1 }}</td>
                                <td>{{$item->nama_kecamatan}}</td>    
                                <td class="text-center">{{number_format($jlh_alh,0,',','.')}}</td>
                                <td class="text-center">{{number_format($jlh_penolong,0,',','.')}}</td>
...
@section('footscript')
    ...
    <script>
		...
		var horizontalBarChartData = {
			...
			datasets: [{
				label: 'Presentase Anak Lahir Hidup',
