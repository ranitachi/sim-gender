                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        @if ($tahun_tersedia->count()!=0)
                                            @foreach ($tahun_tersedia as $item)
                                                <li><a href="{{ route('kependudukan-ditempati.index', [$kategori->id, $item]) }}">{{ $item }}</a></li>
                                            @endforeach
                                        @else
                                            <li><a href="{{ route('kependudukan-ditempati.index', [$kategori->id, date('Y')]) }}">{{ date('Y') }}</a></li>
                                        @endif
                                    </ul>
                                </li>
                        @if (Auth::check())
                                ...
                                <li class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        <li><a href="{{ route('kependudukan-ditempati.create', [$kategori->id, $tahun]) }}">Tambah Data Baru</a></li>
                                        <li><a href="{{ route('kependudukan-ditempati.edit', [$kategori->id, $tahun]) }}">Ubah Data {{ $tahun }}</a></li>
                                    </ul>
                                </li>
                                @endif
                            </ul>
                ...
                <tbody>
                    @foreach ($data as $key => $item)
                        <tr>
                            ...
                            <td>{{ $item->milik_sendiri }}</td>
                            <td>{{ $item->kontrak }}</td>
                            <td>{{ $item->bebas_sewa }}</td>
                            <td>{{ $item->lainnya }}</td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    <script>
		...
		var horizontalBarChartData = {
			...
			datasets: [{
				...
				borderWidth: 1,
				data: [
					@foreach($chart_milik_sendiri as $item)
                        {{ $item }},
                    @endforeach
				]
			}, {
				...
				borderWidth: 1,
				data: [
					@foreach($chart_bebas_sewa as $item)
                        {{ $item }},
                    @endforeach
				]
			}, {
