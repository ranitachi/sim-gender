    ...
    <div class="content">
        <div class="panel panel-flat">
            ...
            <div class="panel-body">
                <form action="{{ route('kependudukan-lansia.update', [$kategori->id, $tahun]) }}" method="post">
                    @csrf
                    @method('put')
                            ...
                            <tbody>
                                @foreach ($data as $key => $item)
                                    <tr>
                                        ...
                                        <td>
                                            <input type="number" min="0" class="form-control" name="terlantar_laki[]" value="{{ $item->terlantar_laki }}" required>
                                        </td>
                                        <td>
                                            <input type="number" min="0" class="form-control" name="terlantar_perempuan[]" value="{{ $item->terlantar_perempuan }}" required>
                                        </td>
                                        <td>
                                            <input type="number" min="0" class="form-control" name="cacat_laki[]" value="{{ $item->cacat_laki }}" required>
                                        </td>
                                        <td>
                                            <input type="number" min="0" class="form-control" name="cacat_perempuan[]" value="{{ $item->cacat_perempuan }}" required>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
