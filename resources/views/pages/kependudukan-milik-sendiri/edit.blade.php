    ...
    <div class="content">
        <div class="panel panel-flat">
            ...
            <div class="panel-body">
                <form action="{{ route('kependudukan-milik-sendiri.update', [$kategori->id, $tahun]) }}" method="post">
                    @csrf
                    @method('put')
                        ...
                        <tbody>
                            @foreach ($data as $key => $item)
                                <tr>
                                    ...
                                    <td>
                                        <input type="number" min="0" class="form-control" name="shm_art[]" value="{{ $item->shm_art }}" required>
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control" name="shm_non_art[]" value="{{ $item->shm_non_art }}" required>
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control" name="shgb[]" value="{{ $item->shgb }}" required>
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control" name="lainnya[]" value="{{ $item->lainnya }}" required>
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control" name="tidak_punya[]" value="{{ $item->tidak_punya }}" required>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
