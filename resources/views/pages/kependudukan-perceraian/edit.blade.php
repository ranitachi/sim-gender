    ...
    <div class="content">
        <div class="panel panel-flat">
            ...
            <div class="panel-body">
                <form action="{{ route('kependudukan-akte.update', [$kategori->id, $tahun]) }}" method="post">
                    @csrf
                    @method('put')
                        ...
                        <tbody>
                            @foreach ($data as $key => $item)
                                <tr>
                                    <td>{{ $key = $key + 1 }}</td>
                                    <td>
                                        <input type="text" class="form-control" readonly name="karakteristik[]" value="{{ $item->jenis_akte }}">
                                    </td>
                                    <td>
                                        <input type="number" min="0" class="form-control" name="jumlah[]" value="{{ $item->jumlah }}" required>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
