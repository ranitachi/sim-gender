...
use App\Models\KesehatanBalitaAsiRatarata;
use Config;

class KesehatanBalitaAsiRatarataController extends Controller
{
    public function index($id_kategori,$tahun=null)
    {
        ...
        $jumlah = KesehatanBalitaAsiRatarata::where('id_kategori', $id_kategori)->where('tahun',$tahun)->get();

        $jenis=Config::get('services.balita_asi');
        ...
        $chart_tpt = array();
        // return $data;
        return view('pages.kesehatan-balitaasi.index')
    ...
    public function create($id_kategori,$tahun)
    {
        ...
        return view('pages.kesehatan-balitaasi.create')
            ->with('kecamatan', $kecamatan)
            ->with('kategori', $kategori)
            ->with('jenis', $jenis)
            ->with('id_kategori', $id_kategori)
    ...
    public function store(Request $request,$id_kategori,$tahun)
    {
        ...
        {
            $ins=new KesehatanBalitaAsiRatarata;
        ...
        return redirect()->route('balita-asi.index',[$id_kategori,$tahun])->with('Data Berhasil Disimpan');
    }
    ...
    public function edit($id_kategori,$tahun)
    {
        ...
        return view('pages.kesehatan-balitaasi.edit')
            ->with('kecamatan', $kecamatan)
            ->with('kategori', $kategori)
            ->with('data', $data)
            ->with('jenis', $jenis)
    ...
    {
        // return $request->all();
        KesehatanBalitaAsiRatarata::where('id_kategori',$id_kategori)->where('tahun',$tahun)->forceDelete();
        ...
        return redirect()->route('balita-asi.index',[$id_kategori,$tahun])->with('Data  Berhasil Diperbaharui');
    }
}
