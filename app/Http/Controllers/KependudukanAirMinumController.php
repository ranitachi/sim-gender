...
use App\Models\KependudukanAirMinum;

use DB;

class KependudukanAirMinumController extends Controller
{
    public function index($id_kategori, $tahun)
    {
        $kategori = Kategori::with('subyek')->findOrFail($id_kategori);

        $data = KependudukanAirMinum::where('id_kategori', $id_kategori)
            ->where('tahun', $tahun)
            ->get();

        $tahun_tersedia = KependudukanAirMinum::select('tahun')->groupby('tahun')->get()->pluck('tahun');

        $chart_karakteristik = KependudukanAirMinum::karakteristik();
        $chart_isi_ulang = [];
        $chart_leding = [];
        $chart_sumur_pompa = [];
        ...
        return view('pages.kependudukan-air-minum.index')
            ->with('data', $data)
            ->with('tahun', $tahun)
    ...
    public function create($id_kategori, $tahun)
    {
        ...
        $karakteristik = KependudukanAirMinum::karakteristik();

        return view('pages.kependudukan-air-minum.create')
    ...
    public function edit($id_kategori, $tahun)
    {
        ...
        return view('pages.kependudukan-air-minum.edit')
            ->with('data', $data)
            ->with('tahun', $tahun)
            ->with('kategori', $kategori);
    }
    ...
    public function update(Request $request, $id_kategori, $tahun)
    {
        DB::transaction(function () use($request, $id_kategori, $tahun) {
            KependudukanAirMinum::where('id_kategori', $id_kategori)->where('tahun', $tahun)->forceDelete();
    
            for ($i=0; $i < count($request->karakteristik); $i++) { 
                $insert = new KependudukanAirMinum;
        ...
        return redirect()->route('kependudukan-air-minum.index', [$id_kategori, $tahun]);
    }
    ...
    public function store(Request $request, $id_kategori, $tahun)
    {
        ...
        return redirect()->route('kependudukan-air-minum.index', [$id_kategori, $request->tahun]);
    }
}
