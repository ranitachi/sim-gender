...
use App\Models\KesehatanPenolongPersalinan;
use Config;

class KesehatanPenolongPersalinanController extends Controller
{
     public function index($id_kategori,$tahun=null)
    {
        ...
        $jumlah = KesehatanPenolongPersalinan::where('id_kategori', $id_kategori)->where('tahun',$tahun)->get();

        $jenis=['tenaga'=>'Tenaga Kesehatan','bukan'=>'Bukan Tenaga Kesehatan'];
        ...
        $chart_tpt = array();
        // return $data;
        return view('pages.kesehatan-penolong.index')
    ...
    public function create($id_kategori,$tahun)
    {
        ...
        return view('pages.kesehatan-penolong.create')
            ->with('krt', $krt)
            ->with('kecamatan', $kecamatan)
            ->with('kategori', $kategori)
            ->with('jenis', $jenis)
    ...
    public function store(Request $request,$id_kategori,$tahun)
    {
        ...
        foreach($kuantal_1 as $id_jen=>$val)
        {
            $ins=new KesehatanPenolongPersalinan;
        ...
        return redirect()->route('penolong-persalinan.index',[$id_kategori,$tahun])->with('Data Berhasil Disimpan');
    }
    ...
    public function edit($id_kategori,$tahun)
    {
        ...
        return view('pages.kesehatan-penolong.edit')
            ->with('kecamatan', $kecamatan)
            ->with('krt', $krt)
            ->with('kategori', $kategori)
            ->with('data', $data)
    ...
    {
        // return $request->all();
        KesehatanPenolongPersalinan::where('id_kategori',$id_kategori)->where('tahun',$tahun)->forceDelete();
        ...
        return redirect()->route('penolong-persalinan.index',[$id_kategori,$tahun])->with('Data  Berhasil Diperbaharui');
    }
}
