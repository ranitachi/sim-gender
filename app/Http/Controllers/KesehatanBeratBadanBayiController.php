...
use App\Models\KesehatanBeratBadanBayi;
use Config;

class KesehatanBeratBadanBayiController extends Controller
{
    public function index($id_kategori,$tahun=null)
    {
        ...
        $jumlah = KesehatanBeratBadanBayi::where('id_kategori', $id_kategori)->where('tahun',$tahun)->get();

        $jenis=['atas_2500'=>'> 2,5 Kg','bawah_2500'=>'< 2,5 Kg','tidak_timbang'=>'Tidak Ditimbang/Tidak Tahu'];
        ...
        $chart_tpt = array();
        // return $data;
        return view('pages.kesehatan-beratbadanbayi.index')
    ...
    public function create($id_kategori,$tahun)
    {
        ...
        return view('pages.kesehatan-beratbadanbayi.create')
            ->with('krt', $krt)
            ->with('kecamatan', $kecamatan)
            ->with('kategori', $kategori)
            ->with('jenis', $jenis)
    ...
    public function store(Request $request,$id_kategori,$tahun)
    {
        ...
        {
            $ins=new KesehatanBeratBadanBayi;
        ...
        return redirect()->route('berat-badan-bayi.index',[$id_kategori,$tahun])->with('Data Berhasil Disimpan');
    }
    ...
    public function edit($id_kategori,$tahun)
    {
        ...
        return view('pages.kesehatan-beratbadanbayi.edit')
            ->with('kecamatan', $kecamatan)
            ->with('krt', $krt)
            ->with('kategori', $kategori)
            ->with('data', $data)
    ...
    {
        // return $request->all();
        KesehatanBeratBadanBayi::where('id_kategori',$id_kategori)->where('tahun',$tahun)->forceDelete();
        ...
        return redirect()->route('berat-badan-bayi.index',[$id_kategori,$tahun])->with('Data  Berhasil Diperbaharui');
    }
}
